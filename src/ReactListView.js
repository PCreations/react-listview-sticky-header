...
export default class ReactListView extends Component {
  ...
  initStickyHeaders() {
    ...
    this.state.events.forEach(type => {
      if (window.addEventListener) {
        ReactDOM.findDOMNode(this.refs.listview).addEventListener(type, this.onScroll.bind(this), false);
      } else {
        ReactDOM.findDOMNode(this.refs.listview).attachEvent('on' + type, this.onScroll.bind(this), false);
      }
    });
  }
  ...
  onScroll() {
    ...
    this.state._instances._originalPositions.forEach((c, index) => {
      ...
      if(!ignoreCheck && (c.originalPosition) < (currentWindowScrollTop + this.state._headerFixedPosition + currentHeaderHeight*index)) {
